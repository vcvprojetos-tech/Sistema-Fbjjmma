...
export async function POST(
  ...
  const existing = (match.callTimes as { call: number; pos?: string | null }[] | null) ?? []
  // Só pula se o painel já registrou o marcador (pos: null) — chamadas do coordenador (pos: p1/p2) não contam
  if (existing.some(c => c.call === 1 && (c.pos === null || c.pos === undefined))) {
    return NextResponse.json({ ok: true, skipped: true })
  }

  await prisma.match.update({
    where: { id: matchId },
    data: { callTimes: [...existing, { call: 1, at: new Date().toISOString(), pos: null }] },
  })
